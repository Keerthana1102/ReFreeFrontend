...
class Favourites extends React.Component{
	...
	async componentDidMount(){
        ...
        let allfollows = [];
	    const response = await axios({url:`http://127.0.0.1:8000/follow/userfollows`,method:'GET' ,params:{userId:this.state.userId}, withCredentials:true}).then(response=>{return response}).catch(error=>{window.location.href="http://127.0.0.1:3000/"})
        const json = await response.data;
        console.log(json);
        json.map(user => allfollows.push(user));
        console.log(allfollows);
        let fav = [];
        for(let user of allfollows)
        {
            console.log(user);
            const res = await axios({url:`http://127.0.0.1:8000/users/${user.following_user_id}`,method:'GET',withCredentials:true}).then(response=>{return response})
            fav.push(res.data);
        }
        console.log(fav)
	    this.setState({persons:fav});
	}
			...
			<Grid padded>
	          <Grid.Row width = {4}>
		          ...
		          <Header as='h2'>
		            ...
		            <Header.Subheader>Search for Favourites</Header.Subheader>
		            </Header.Content>
		         </Header>
		        </div>
	          </Grid.Row>
